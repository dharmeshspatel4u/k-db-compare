    ...
    if (typeof targetDataQuery == 'undefined') {
      ...
      sourceDataQuery.data.map((currentDataPoint) => {
        ...
        if (typeof targetDataPoint == 'undefined') {
          ...
          let dataPointDiffs = getDataPointDiffs(sourceDataQuery.name, currentDataPoint[query.keyColumn], currentDataPoint, targetDataPoint)
...
let getDataPointDiffs = (configName, key, source, target) => {
  let dataPointDiffs = []
  for (let prop in source) {
    if (typeof target == 'undefined' || typeof target[prop] == 'undefined') {
      dataPointDiffs.push({ comparisonType: 'data-config-item', sourceObject: configName + '.' + key + '.' + prop + '.' + source[prop], targetObject: configName + '.' + key + '.' + prop + '.undefined' })
    } else {
      if (source[prop] != target[prop]) {
        dataPointDiffs.push({ comparisonType: 'data-config-item', sourceObject: configName + '.' + key + '.' + prop + '.' + source[prop], targetObject: configName + '.' + key + '.' + prop + '.' + target[prop] })
      }
    }
  }
